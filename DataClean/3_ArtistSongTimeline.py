...
date = pd.date_range('3/1/2015','10/30/2015')
columns = ['artist_id','song_id','publish_time','tmp','isold']+[x.strftime('%Y-%m-%d') for x in date]
...
for i,x in enumerate(date):
    index = x.strftime('%Y-%m-%d');
    frame[index] = frame['tmp']+i
